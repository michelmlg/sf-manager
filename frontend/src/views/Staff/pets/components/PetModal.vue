...
import PetAdoptionRequestTab from '@/views/Staff/pets/components/PetAdoptionRequestTab.vue';
...
<template>
  ...
  >
    ...
    >
      ...
      <div class="w-1/3 bg-ong-background p-6 flex flex-col border-r">
        <!-- Carousel fixo -->
        <div class="flex-none mb-4">
          <!-- <PetMediaCarousel :pet-id="animal.id" /> -->
          <PetMediaCarousel :pet-id="animal.id"/>
        </div>

        <!-- Campos editáveis roláveis -->
        <div class="flex-1 overflow-y-auto space-y-4">
          <!-- Nome -->
          <div v-if="!isEditing" class="">
            <h2 class="text-2xl font-heading text-ong-text">{{ editableAnimal.name }}</h2>
            <p class="text-muted-foreground">
              <template v-if="editableAnimal.species?.name">{{  editableAnimal.species.name }}</template>
              <p v-else>Espécie não informada</p>
              <span> • </span>
              <template v-if="editableAnimal.breed?.name">{{ editableAnimal.breed.name }}</template>
              <p v-else>Raça não informada</p>
            </p>
          </div>
          ...
          <div class="mt-4 text-sm w-full">
            ...
            <div v-if="!isEditing">
              <p class="text-ong-text line-clamp-6">{{ editableAnimal.notes }}</p>
            </div>
            <div v-else>
              <textarea
      ...
      <div class="w-2/3 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div v-else-if="activeTab === 'Pedidos de Adoção'">
            <PetAdoptionRequestTab :pet-id="animal.id"/>
          </div>
